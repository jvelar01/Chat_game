...
class Player2():
    def __init__(self, side):
        self.side = side
        if side == LEFT_PLAYER:
            self.pos = [30, SIZE2[Y]//2]
        else:
            self.pos = [SIZE2[X] - 30, SIZE2[Y]//2]
...
class Game2():
    def __init__(self, manager):
        self.players = manager.list([Player2(LEFT_PLAYER),Player2(RIGHT_PLAYER)])
